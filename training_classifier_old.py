...
def optimization_loss(logits: torch.Tensor, ground_truth_labels: torch.Tensor, is_binary: bool, weights: torch.Tensor):
    if is_binary:
        gt_float = ground_truth_labels.to(torch.float32)
        sample_weights = gt_float * weights + 1
        return torch.sum(sample_weights *
                         torch.nn.functional.binary_cross_entropy_with_logits(logits, gt_float, reduction="none")), torch.sum(sample_weights).item()
...
def single_training_step(model_: torch.nn.Module, optimizer_: torch.optim.Optimizer,
    ...
    logits = model_(img_sample_batch_.squeeze(2))
    preds = {}
    losses = {}
    loss = 0
    for key in labels_batch_:
        ...
        opt_loss, weight_sum = optimization_loss(logits, gt, is_binary=True, weights=weights)
        losses[key] = {"loss": opt_loss, "weight_sum": weight_sum}
        preds[key] = (logits > 0.0).to(torch.long)
...
if __name__ == "__main__":
    ...
    ternary_collapsed_binary_weight = torch.tensor(2.0, dtype=torch.float32, device=config.device)
    ...
    used_labels = {"bowel": False, "extravasation": False, "kidney": 1, "liver": 0, "spleen": 0}
    bowel, extravasation, kidney, liver, spleen = used_labels["bowel"], used_labels["extravasation"], used_labels["kidney"], used_labels["liver"], used_labels["spleen"]
    ...
    model = model_resnet_old.FullyBinaryClassifier(backbone=backbone, channels=hidden_channels * (2 ** (len(hidden_blocks) - 1)))
